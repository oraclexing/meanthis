...
- 保持浮动挂件与侧栏之间的捕捉数量、删除结果和任务说明编辑一致，包括已保存站点清理与保存失败后的恢复。
- 打开设置时显示实际捕捉状态，在连接更新期间保留滚动位置，并柔化浮动控件的边缘和阴影。
- 通过显式、有界的页面观察，对比已保存捕捉与当前页面。
- 减少桥接启动时的重复检查，合并并发状态刷新，避免定时请求在慢操作后积压。
...
- 可选 MCP broker 在 POSIX 输出管道暂时写满时保留完整响应，并限制重试时间；输出受阻时的关闭行为已验证。
